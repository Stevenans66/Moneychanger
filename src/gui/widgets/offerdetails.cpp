...
//    }
//    // ---------------------------------------------------
//    QString qstrMarketNymName = QString::fromStdString(ot.Exec().GetNym_Name(qstrMarketNymID.toStdString()));
...
//    }
//    else
//        qstrMarketServerName = QString::fromStdString(ot.Exec().GetServer_Name(qstrMarketNotaryID.toStdString()));
//    // ---------------------------------------------------
...
//        const int64_t lTotalAsset            (Moneychanger::stringToAmount(str_asset_id,           //...and lTotalAsset is 9000
...
//        const int64_t lScale                 (Moneychanger::stringToAmount(str_asset_id,                 // ...then lScale is 1000
//                                                                         qstrScale  .toStdString   ()));
//        // --------------------------------------------
//        const QString qstrPrice              (theWizard.field("pricePerScale")      .toString      ());// If this is "$ 45.98"
//        const int64_t lPrice                 (Moneychanger::stringToAmount(str_currency_id,              // ...then lPrice is 4598
...
//        // --------------------------------------------
//        const int64_t lAssetAcctBalance      (Moneychanger::stringToAmount(str_asset_id,
//                                                                         qstrAssetAcctBalance   .toStdString()));
//        const int64_t lCurrencyAcctBalance   (Moneychanger::stringToAmount(str_currency_id,
...
//            {
//                const QString qstrNeededBalance = QString::fromStdString(ot.Exec().FormatAmount(
...
//            // --------------------------------------------------------------
//            auto action = ot.ServerAction().CreateMarketOffer(opentxs::Identifier::Factory(str_asset_acct_id),
...
//                    MTSpinner theSpinner;
//                    // --------------------------------------------------------------
//                    auto action = ot.ServerAction().KillMarketOffer(opentxs::Identifier::Factory(str_nym_id),
...
//    // -------------------------------------------
//    if (qstr_current_id.isEmpty() && (ot.Exec().GetServerCount() > 0))
//        qstr_current_id = QString::fromStdString(ot.Exec().GetServer_ID(0));
//    // -------------------------------------------
...
//    {
//        QString OT_id = QString::fromStdString(ot.Exec().GetServer_ID(ii));
//        QString OT_name("");
//        // -----------------------------------------------
//        if (!OT_id.isEmpty())
//        {
...
//                bFoundDefault = true;
//            // -----------------------------------------------
//            OT_name = QString::fromStdString(ot.Exec().GetServer_Name(OT_id.toStdString()));
//            // -----------------------------------------------
...
//                    // ------------------------------------------------------
//                    int64_t     lTotalAssets      = ot.Exec().StringToLong(pOfferData->total_assets);
//                    int64_t     lFinished         = ot.Exec().StringToLong(pOfferData->finished_so_far);
//                    // ------------------------------------------------------
//                    int64_t     lStillAvailable   = lTotalAssets - lFinished;
//                    // ------------------------------------------------------
//                    int64_t     lMinimumIncrement = ot.Exec().StringToLong(pOfferData->minimum_increment);
//                    // ------------------------------------------------------
//                    int64_t     lScale            = ot.Exec().StringToLong(pOfferData->scale);
//                    std::string str_scale         = ot.Exec().FormatAmount(pOfferData->instrument_definition_id, lScale);
//                    // ------------------------------------------------------
//                    int64_t     lPrice            = ot.Exec().StringToLong(pOfferData->price_per_scale);
//                    std::string str_price         = ot.Exec().FormatAmount(pOfferData->currency_type_id, lPrice);
//                    // ------------------------------------------------------
...
//                    // ------------------------------------------------------
//                    QString qstrMinimumIncrement  = QString::fromStdString(ot.Exec().FormatAmount(pOfferData->instrument_definition_id, lMinimumIncrement));
//                    QString qstrTotalAssets       = QString::fromStdString(ot.Exec().FormatAmount(pOfferData->instrument_definition_id, lTotalAssets));
//                    QString qstrSoldOrPurchased   = QString::fromStdString(ot.Exec().FormatAmount(pOfferData->instrument_definition_id, lFinished));
//                    QString qstrStillAvailable    = QString::fromStdString(ot.Exec().FormatAmount(pOfferData->instrument_definition_id, lStillAvailable));
//                    // ------------------------------------------------------
//                    std::string str_asset_name    = ot.Exec().GetAssetType_Name(pOfferData->instrument_definition_id);
//                    // -----------------------------------------------------------------------
//                    time_t tValidFrom      = static_cast<time_t>(ot.Exec().StringToLong(pOfferData->valid_from));
//                    time_t tValidTo        = static_cast<time_t>(ot.Exec().StringToLong(pOfferData->valid_to));
//                    // -----------------------------------------------------------------------
...
//                    //
//                    // ------------------------------------------------------
//                    ui->lineEditAssetAcct      ->setText(QString::fromStdString(ot.Exec().GetAccountWallet_Name(pOfferData->asset_acct_id)));
//                    ui->lineEditCurrencyAcct   ->setText(QString::fromStdString(ot.Exec().GetAccountWallet_Name(pOfferData->currency_acct_id)));
//                    // ------------------------------------------------------
...
//                    // ------------------------------------------------------
//                    const int64_t lAcctBalance     = ot.Exec().GetAccountWallet_Balance(pOfferData->asset_acct_id);
//                    const int64_t lCurrencyBalance = ot.Exec().GetAccountWallet_Balance(pOfferData->currency_acct_id);

//                    const std::string str_acct_balance     = ot.Exec().FormatAmount(pOfferData->instrument_definition_id,    lAcctBalance);
//                    const std::string str_currency_balance = ot.Exec().FormatAmount(pOfferData->currency_type_id, lCurrencyBalance);
    ...
    if (OFFER_MAP.end() != it_offer)
    {
        ...
        if (NULL != pOfferData) // Should never be NULL.
        {
            ...
            if (pModel && pTradeDataProxy_)
            {
                ...
                int64_t lOfferID = opentxs::String::StringToLong(pOfferData->transaction_id);
...
//    this->blockSignals(true);
    ...
    if (OFFER_MAP.end() != it_offer)
    {
        ...
        if (NULL != pOfferData) // Should never be NULL.
        {
            ...
            std::string   str_server_display = ot.Exec().GetServer_Name(str_server);
            ...
            {
                const std::string str_price_per_scale(ot.Exec().FormatAmount(pOfferData->instrument_definition_id,
                                                                               lScale));
            ...
            if (NULL != pTradeList)
            {
                ...
                for (size_t trade_index = 0; trade_index < nTradeDataCount; ++trade_index)
                {
                    ...
                    // -----------------------------------------------------------------------
                    int64_t lOfferID = ot.Exec().StringToLong(pOfferData->transaction_id);
                    int64_t lTradeID = ot.Exec().StringToLong(pTradeData->transaction_id);
...
//                  time_t tDate = static_cast<time_t>(ot.Exec().StringToLong(pTradeData->date));
                    time64_t tDate = static_cast<time64_t>(ot.Exec().StringToLong(pTradeData->date));
                    ...
                    int64_t       lPrice            = ot.Exec().StringToLong(str_price); // this price is "per scale"

                    if (lPrice < 0)
                        lPrice *= (-1);

                    std::string   str_price_display = ot.Exec().FormatAmount(pOfferData->currency_type_id, lPrice);

                    QString qstrPrice = QString::fromStdString(str_price_display);
                    // -----------------------------------------------------------------------
                    std::string & str_amount_sold    = pTradeData->amount_sold;
                    int64_t       lQuantity          = ot.Exec().StringToLong(str_amount_sold); // Total amount of asset sold.

                    if (lQuantity < 0)
                        lQuantity *= (-1);

                    std::string   str_amount_display = ot.Exec().FormatAmount(pOfferData->instrument_definition_id, lQuantity);

                    QString qstrAmountSold = QString::fromStdString(str_amount_display);
                    // -----------------------------------------------------------------------
                    std::string & str_currency_paid   = pTradeData->currency_paid;
                    int64_t       lPayQuantity        = ot.Exec().StringToLong(str_currency_paid); // Total currency paid

                    if (lPayQuantity < 0)
                        lPayQuantity *= (-1);

                    std::string   str_paid_display    = ot.Exec().FormatAmount(pOfferData->currency_type_id, lPayQuantity);
                    ...
                    if (0 == lPrice)
                    {
                        if ((lQuantity != 0) && (lPayQuantity != 0) && (lScale != 0))
                        {
                            ...
                            str_price_display = ot.Exec().FormatAmount(pOfferData->currency_type_id, lPrice);
