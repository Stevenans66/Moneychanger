...
//        }
//        // --------------------------------------
//        std::string str_InstrumentDefinitionID(ot.Exec().GetAccountWallet_InstrumentDefinitionID(m_myAcctId.toStdString()));
...
//            int64_t     amount               = ot.Exec().StringToAmount(str_InstrumentDefinitionID, str_temp);
//            std::string str_formatted_amount = ot.Exec().FormatAmount(str_InstrumentDefinitionID, static_cast<int64_t>(amount));
...
//    QString     qstr_fromNymId(QString::fromStdString(ot.Exec().GetAccountWallet_NymID(str_fromAcctId)));
...
//        initial_amount = ot.Exec().StringToAmount(str_InstrumentDefinitionID, str_initial_amount);
//        // ----------------------------------------------------
...
//        recurring_amount = ot.Exec().StringToAmount(str_InstrumentDefinitionID, str_recurring_amount);
//    }
//    // ----------------------------------------------------
//    if (initial_amount < 0)
//    {
...
//    const std::string myNymId  = ot.Exec().GetAccountWallet_NymID(myAcctId);
//    const std::string notaryId = ot.Exec().GetAccountWallet_NotaryID(myAcctId);
...
//    {
//        if (!ot.ServerAction().GetTransactionNumbers(myNymID, notaryID, 2))
//        {
...
//    // ------------------------------------------------------------
//    const std::string str_plan = ot.Exec().ProposePaymentPlan(
//        notaryId,
//        timeFrom,
...
//        auto action = ot.ServerAction().SendPayment(myNymID, notaryID,
...
//        ot.Exec().Msg_HarvestTransactionNumbers(str_plan, myNymId, false, false, false, false, false);
//        // ------------------------------------
...
//                bFoundDefault = true;
//            // -----------------------------------------------
//            OT_acct_name = QString::fromStdString(ot.Exec().GetAccountWallet_Name(OT_acct_id.toStdString()));
//            // -----------------------------------------------
...
//        {
//            str_my_name = ot.Exec().GetAccountWallet_Name(m_myAcctId.toStdString());
...
//        {
//            str_his_name = ot.Exec().GetNym_Name(m_hisNymId.toStdString());
